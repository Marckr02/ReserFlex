...
function SkeletonLoader({ theme, dark }) {
            ...
            <div key={i} className={`rounded-2xl border p-6 ${surface}`}>
              <div className="flex justify-between items-start gap-4">
                ...
                <div className="space-y-2 text-right">
                  <div className={`h-8 w-16 rounded-xl ${line} animate-pulse ml-auto`}></div>
                  <div className={`h-5 w-20 rounded-full ${line} animate-pulse ml-auto`}></div>
                </div>
              </div>
            </div>
          ))}
...
export default function Catalogo() {
  ...
  if (loading) {
    return <SkeletonLoader theme={theme} dark={dark} />;
  }
  ...
  return (
    <div className={`min-h-screen pb-16 ${dark ? 'bg-neutral-950' : 'bg-stone-50'}`}>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <div className={`flex items-center gap-2 mt-3 text-sm ${dark ? 'text-slate-300' : 'text-stone-500'}`}>
                <svg
                  className={`w-4 h-4 shrink-0 ${dark ? 'text-slate-400' : 'text-stone-400'}`}
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  viewBox="0 0 24 24"
      ...
      <div className={`mx-auto max-w-3xl px-4 py-6 ${dark ? '' : ''}`}>
        <div className={`rounded-2xl border p-4 shadow-sm ${dark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-stone-200/80'}`}>
          <div className="relative">
            <svg
              className={`absolute left-4 top-1/2 h-5 w-5 ${dark ? 'text-slate-500 -translate-y-1/2' : 'text-stone-400 -translate-y-1/2'}`}
              fill="none"
              stroke="currentColor"
              strokeWidth="2"
              viewBox="0 0 24 24"
            ...
            <input
              ...
              className={`w-full rounded-xl border pl-12 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-stone-300 focus:border-transparent transition ${
                dark
                  ? 'border-neutral-800 bg-neutral-900 text-slate-100 placeholder:text-slate-500'
          ...
          <div className={`mt-3 flex flex-wrap gap-2 items-center`}>
            <select
              ...
              className={`appearance-none rounded-lg border pl-3 pr-8 py-2 text-xs font-semibold focus:outline-none transition-colors cursor-pointer ${
                dark
                  ? 'border-neutral-800 bg-neutral-900 text-slate-200 hover:border-neutral-700'
      ...
      <div className={`mx-auto max-w-3xl px-4 ${dark ? '' : ''}`}>
        {filteredServices.length === 0 ? (
          <div className={`rounded-2xl border p-10 text-center shadow-sm ${dark ? 'bg-neutral-900 border-neutral-800' : 'bg-white border-stone-200/80'}`}>
            ...
            <p className={`mt-1 text-sm ${dark ? 'text-slate-300' : 'text-stone-500'}`}>
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-start gap-4">
                  <div className="flex-1 min-w-0">
                    <h2 className={`text-xl font-bold leading-tight ${dark ? 'text-slate-100' : 'text-stone-900'}`}>
                      {service.name}
                    </h2>
                    {service.description && (
                      <p className={`mt-2 text-sm leading-relaxed line-clamp-2 ${dark ? 'text-slate-300' : 'text-stone-500'}`}>
                        {service.description}
                      </p>
                    )}
                      ...
                      {service.employeeServices?.length > 0 && (
                        <>
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            profesional{service.employeeServices.length !== 1 ? 'es' : ''}
                          </span>
                        </>
                      )}
                    </div>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-3 sm:gap-1 shrink-0">
                    <div className="text-right">
                      <p className={`text-[10px] uppercase tracking-wider font-bold ${dark ? 'text-slate-400' : 'text-stone-400'}`}>
                        Desde
                      </p>
                      <p className={`text-2xl font-black ${dark ? 'text-slate-100' : 'text-stone-900'}`}>
                        ${service.price}
                      </p>
                    </div>
                    <Link
