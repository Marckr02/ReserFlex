...
const createBusiness = async (req, res) => {
  try {
    ...
    if (existingBusiness) return res.status(409).json({ message: 'Ya existe un negocio con ese nombre' });

    const business = await prisma.business.create({
      data: { name, slug, type, address, logoUrl }
    });
    ...
    await prisma.user.create({
      data: { name: adminName, email: adminEmail, password: hashedPassword, role: 'ADMIN_NEGOCIO', verified: true, businessId: business.id }
    });
...
const getAllBusinesses = async (req, res) => {
  try {
    const businesses = await prisma.business.findMany({
      select: { id: true, name: true, slug: true, type: true, active: true, createdAt: true },
...
const getBusinessBySlug = async (req, res) => {
  try {
    ...
    const business = await prisma.business.findFirst({
      ...
      select: { id: true, name: true, type: true, address: true, logoUrl: true }
    });
...
module.exports = { createBusiness, getAllBusinesses, getPublicBusinesses, getBusinessBySlug, toggleBusiness, checkSlug };
