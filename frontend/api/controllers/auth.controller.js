...
const login = async (req, res) => {
  try {
    ...
    if (!validPassword) return res.status(401).json({ message: 'Credenciales no válidas' });

    const token = jwt.sign(
      { id: user.id, role: user.role, email: user.email, name: user.name, businessId: user.businessId || null },
